...
case class Span(
  ...
  lazy val serviceName: Option[String] = {
    // Most authoritative is the label of any server annotation, logged by an instrumented server
    serverSideAnnotations.map(_.serviceName).filterNot(_.isEmpty).headOption orElse
      ...
      clientSideAnnotations.map(_.serviceName).filterNot(_.isEmpty).headOption orElse
